...
public class DweetProLib {
    ...
    private final static String TAG = "DweetProLib";

    private static DweetProLib instance;
    ...
    public static DweetProLib getInstance(Context ctx) {
        currentCtx = ctx;
        return DweetProLib.instance;
    }

    public interface DweetProCallback {
        void callback(ArrayList<Object> ar);
    }

    public String sendDweet (JSONObject data,String token,Object caller,DweetProCallback cb,boolean overwrite) {

        final String JSONString = data.toString();

        final String urlstr = "https://openxc.dweet.io/v2/dweets";
        ...
        if (dt==null) {
            ...
            try {
                ...
                conn.setRequestProperty("X-DWEET-AUTH", token);
    ...
    private class DweetTask extends AsyncTask<Object,String,Integer> {
        ...
        protected void onPostExecute(Integer result) {
            ...
            if (thingProcessCaller.get(this)!=null) {
                DweetProCallback dc = (DweetProCallback)thingProcessCallback.get(this);
                ArrayList ar = new ArrayList<>();
                ar.add(result);
                dc.callback(ar);
            }
